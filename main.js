...
// renderer, copied into feelreef's GAIDO bar): wrap each markdown MARKER in a hidden span
...
// namespace-neutral: the tile plugins use `tg-*`, feelreef's GAIDO bar uses `gd-*`, a preview
...
//   • gaido_bar.py inlineMd (gd-b / gd-i / gd-code) — markers in gd-mk
...
// pass directly. (The gaido_bar copy required callers to pre-escape — `inlineMd(esc(...))`.
...
//   prefix  class namespace, default 'tg'. tile → 'tg', GAIDO → 'gd', preview → 'gp'.
//
// Behaviour (faithful to editor-core.js highlightLineParts + gaido_bar.py inlineMd):
...
//     (the GAIDO bar tints code teal, marktile uses its own accent); this primitive ships only
...
//     mirroring gaido_bar.py's `.gd-answer .gd-mk{display:none}`.
